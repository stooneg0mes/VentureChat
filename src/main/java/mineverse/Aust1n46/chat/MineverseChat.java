    ...
    public void onPluginMessageReceived(String channel, Player player, byte[] inputStream) {
        ...
        try {
            ...
            if (subchannel.equals("Chat")) {
                ...
                int firstHash = msgin.readInt();
                String firstFormat = msgin.readUTF();
                String firstGlobalJSON = msgin.readUTF();

                int secondHash = msgin.readInt();
                String secondFormat = msgin.readUTF();
                String secondGlobalJSON = msgin.readUTF();

                String consoleChat = firstFormat + " " + secondFormat + chat;
                ...
                Bukkit.getServer().getScheduler().runTaskAsynchronously(this, new Runnable() {
                    ...
                    public void run() {
                        ...
                        VentureChatEvent ventureChatEvent = new VentureChatEvent(null, senderName, nickname, primaryGroup, chatChannelObject, recipients, recipients.size(), chat, firstFormat, firstGlobalJSON, firstHash, secondFormat, secondGlobalJSON, secondHash, false);
                ...
                for (MineverseChatPlayer p : MineverseChatAPI.getOnlineMineverseChatPlayers()) {
                    if (p.isListening(chatChannelObject.getName())) {
                        ...
                        String firstJson = Format.formatModerationGUI(firstGlobalJSON, p.getPlayer(), senderName, chatchannel, firstHash);
                        PacketContainer firstPacket = Format.createPacketPlayOutChat(firstJson);

                        String secondJSON = Format.formatModerationGUI(secondGlobalJSON, p.getPlayer(), senderName, chatchannel, secondHash);
                        ...
                        if (getConfig().getBoolean("ignorechat", false)) {
                            if (!p.getIgnores().contains(senderUUID)) {
                                ...
                                Format.sendPacketPlayOutChat(p.getPlayer(), firstPacket);
                                Format.sendPacketPlayOutChat(p.getPlayer(), secondPacket);
                            }
                            continue;
                        }
                        Format.sendPacketPlayOutChat(p.getPlayer(), firstPacket);
